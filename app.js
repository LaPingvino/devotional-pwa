            ...
            let browserLang = (navigator.language || navigator.userLanguage || 'en').split('-')[0].toLowerCase();
            ...
            async function fetchLanguageNames() {
                ...
                const fetchPromise = executeQuery(sql)
                    .then(rows => {
                        ...
                        if (rows && Array.isArray(rows)) {
                            ...
                            try {
                                localStorage.setItem(LANGUAGE_NAMES_CACHE_KEY, JSON.stringify({ timestamp: Date.now(), data: languageNamesMap }));
                                // console.log("Fetched and cached language names.");
                            } catch (e) {
                                console.error("Error caching language names:", e);
                            }
                        } else {
                             console.warn("No rows returned from language names query or invalid format.");
                        }
                ...
                const timeoutPromise = new Promise((resolve) => {
                    setTimeout(() => {
                        if (!fetchCompleted) {
                            console.warn(`Fetching language names timed out after ${FETCH_LANG_NAMES_TIMEOUT_MS / 1000}s. Proceeding with fallbacks.`);
            ...
            function updatePrayerMatchingToolDisplay() {
                ...
                const itemsListUl = document.getElementById( // Renamed from matchesListUl
                    "collected-items-list",
                );
                const reviewAndSendButton = document.getElementById( // Was doltHubIssueButton
                    "review-and-send-button",
                );
                const clearAllLink = document.getElementById( // Was clearMatchesButton
                ...
                if (pinnedPrayerDetails) {
                    ...
                    if (pinnedPrayerDetails.language) {
                        let pageState =
                            currentPageByLanguage[pinnedPrayerDetails.language];
                ...
                if (hasItems) {
                    collectedMatchesForEmail.forEach((itemData, index) => { // Renamed from matchData
                        ...
                        removeButton.onclick = () =>
                            removeCollectedMatch(index); // "Match" in function name is fine
                ...
                if (clearAllLink) {
                    const hasSomethingToClear = collectedMatchesForEmail.length > 0 || pinnedPrayerDetails;
                    clearAllLink.style.display = hasSomethingToClear ? 'inline-block' : 'none';
                }


                if (typeof componentHandler !== "undefined") {
                    if (reviewAndSendButton && reviewAndSendButton.classList.contains("mdl-js-button")) {
            ...
            function generateDoltHubIssueBody() {
                ...
                collectedMatchesForEmail.forEach((matchData, index) => {
                    bodyLines.push(
                        `**Suggestion ${index + 1} (Type: ${matchData.type}):**`,
                    );
                    ...
                    if (matchData.type === "match_prayers") {
                        ...
                        bodyLines.push(
                            `- Suggested Phelps: ${matchData.newPhelps}`,
                        );
                    } else if (matchData.type === "change_language") {
                        bodyLines.push(
                ...
                if (sqlUpdateQueries.length > 0) {
                    ...
                    bodyLines.push(
                        "--- No SQL UPDATE Statements Suggested ---",
                    );
                }
                bodyLines.push("");
                bodyLines.push(
            ...
            async function renderPrayer(
                ...
                for (const match of collectedMatchesForEmail) {
                    if (
                        match.prayer &&
                        match.prayer.version === prayer.version
                    ) {
                ...
                if (pinnedPrayerDetails) {
                        ...
                        replacePinButton.onclick = () => {
                            ...
                            if (
                                ...
                                snackbarContainer.MaterialSnackbar.showSnackbar(
                                    {
                                        message:
                                            "Pinned prayer replaced. Item list preserved.",
                                    },
                                );
                            }
                        };
                    ...
                    pinButton.onclick = () => {
                        ...
                        if (
                            ...
                            snackbarContainer.MaterialSnackbar.showSnackbar({
                                message:
                                    "Prayer pinned! Navigate to find items or suggestions.",
                            });
                        }
                    };
                ...
                if (typeof componentHandler !== "undefined") {
                    ...
                    Array.from(
                        actionsDiv.querySelectorAll(".mdl-js-button"),
                    ).forEach((btn) => {
                        if (btn.MaterialButton)
            ...
            document.addEventListener("DOMContentLoaded", () => {
                ...
                const reviewAndSendButton = document.getElementById( // Was dolthub-issue-button
                    "review-and-send-button",
                );
                ...
                const clearAllItemsLink = document.getElementById( // Was clear-collected-matches-button
                    "clear-all-items-link",
                );
                if (clearAllItemsLink) {
                    clearAllItemsLink.addEventListener("click", (e) => {
                        ...
                        if (confirm("Are you sure you want to clear all collected items and unpin the current prayer (if any)? This cannot be undone.")) {
                ...
                const sqlTextarea = document.getElementById(
                    "sql-display-textarea",
                );
                const copySqlButton =
                    document.getElementById("copy-sql-button");
                    ...
                    copySqlButton.addEventListener("click", () => {
                        ...
                        try {
                            navigator.clipboard
                                ...
                                .catch((err) => {
                                    console.error(
                                        "Failed to copy with navigator.clipboard: ",
                                        err,
                                    );
                ...
                if (
                    ...
                    closeSqlDialogButton.addEventListener("click", () =>
                        sqlDialog.close(),
                    );
                ...
                const dolthubIssueDialog = document.getElementById(
                ...
                const dialogOpenSqlDoltHubButton = document.getElementById("dialog-open-sql-doltub-button");
                ...
                if (openNewDoltHubIssuePageButton) {
                    openNewDoltHubIssuePageButton.addEventListener(
                        "click",
                        () => {
                            window.open(
                                DOLTHUB_REPO_ISSUES_NEW_URL_BASE,
                                "_blank",
                            );
                            if (
                                snackbarContainer &&
                                ...
                                snackbarContainer.MaterialSnackbar.showSnackbar(
                                    {
                                        message:
                                            "DoltHub new issue page opened. Paste the copied content.",
                                        timeout: 5000,
                                    },
                                );
                        },
                ...
                if (
                    ...
                    closeDoltHubIssueDialogButton.addEventListener(
                        "click",
